...
# Complete property returned by the API.
# It includes the ID generated by the backend.
...
# Data required from the client when creating a property.
# It does not include an ID because the backend generates it.
...
@app.get("/")
def root():
    return {"message": "Welcome to Hash4 API"}
